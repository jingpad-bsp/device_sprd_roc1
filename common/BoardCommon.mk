...
ifeq (--board_compatible,$(strip $(BOARD_MEMORY_COMPATIBLE)))
TARGET_RECOVERY_FSTAB_COMPATIBLE_EMMC := recovery.emmc$(RECOVERY_FSTAB_SUFFIX).fstab
TARGET_RECOVERY_FSTAB_COMPATIBLE_UFS := recovery.ufs$(RECOVERY_FSTAB_SUFFIX).fstab
else
TARGET_RECOVERY_FSTAB_COMPATIBLE :=
endif # endif compatiible device suport ufs and emmc device
ifeq (ufs,$(strip $(BOARD_MEMORY_TYPE)))
  RECOVERY_FSTAB_MEMORY_SUFFIX := .ufs
else
  RECOVERY_FSTAB_MEMORY_SUFFIX := .emmc
endif
TARGET_RECOVERY_FSTAB := $(RECOVERYCOMM)/recovery$(RECOVERY_FSTAB_MEMORY_SUFFIX)$(RECOVERY_FSTAB_SUFFIX).fstab
