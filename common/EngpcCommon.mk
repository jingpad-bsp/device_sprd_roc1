...
ifeq (ufs,$(strip $(BOARD_MEMORY_TYPE)))
  RECOVERY_FSTAB_MEMORY_SUFFIX := .ufs
else
  RECOVERY_FSTAB_MEMORY_SUFFIX := .emmc
endif

PRODUCT_COPY_FILES += \
    $(LOCAL_PATH)/rootdir/root/init.cali.rc:$(TARGET_COPY_OUT_VENDOR)/etc/init/hw/init.cali.rc \
    $(LOCAL_PATH)/rootdir/root/fstab$(RECOVERY_FSTAB_MEMORY_SUFFIX).cali:$(TARGET_COPY_OUT_VENDOR)/etc/fstab.cali \
    $(LOCAL_PATH)/rootdir/root/init.factorytest.rc:$(TARGET_COPY_OUT_VENDOR)/etc/init/hw/init.factorytest.rc \
    $(LOCAL_PATH)/rootdir/root/fstab$(RECOVERY_FSTAB_MEMORY_SUFFIX).factorytest:$(TARGET_COPY_OUT_VENDOR)/etc/fstab.factorytest \
